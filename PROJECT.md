...
> **Last updated:** Prompt 103 — Vorkasse/Banküberweisung payment method + Resend order emails.
...
| Prompt | Changes |
|--------|--------|
| 103 | **Vorkasse / Banküberweisung payment method + transactional emails via Resend.** User: *"napravi mi sistem sa prednarudjbom pored ebay placanja"* — preorder (bank transfer) next to the existing PayPal payment; emails only for preorders via free Resend. Changes: (1) [src/types/index.ts](src/types/index.ts) — `Order.status` extended with `"awaiting_payment"` (Vorkasse: customer must transfer money first), new `PaymentMethod = "paypal" \| "bank_transfer"` type, `Order.paymentMethod` + `Order.email?` fields added. (2) New [src/lib/bank.ts](src/lib/bank.ts) — `BANK_ACCOUNT` constant with **FAKE** placeholder details (holder „MYICON GmbH“, IBAN `DE12 3456 7890 1234 5678 90`, BIC `MYICDEFFXXX`, „Musterbank eG“, 7-day payment deadline) + `paymentReference(orderId)` helper → `MYICON-<orderId>` Verwendungszweck. Replace before going live. (3) New [src/lib/email.ts](src/lib/email.ts) — Resend REST API integration (`POST https://api.resend.com/emails` with `VITE_RESEND_API_KEY`), German-branded HTML shell, `sendOrderConfirmationEmail(order)` (order + item table + IBAN/BIC/Verwendungszweck + deadline) and `sendPaymentReceivedEmail(order)` („Zahlung ist eingegangen“). Browser-side call with graceful `console.info` fallback when no key configured — email never blocks the order flow. SECURITY NOTE in file: key is visible in the client bundle; for production move sending to a serverless function. (4) [CheckoutPage.tsx](src/pages/CheckoutPage.tsx) — payment method radio selector (PayPal vs Banküberweisung/Vorkasse), PayPal keeps the 1.2s fake delay, bank transfer skips it; order status `awaiting_payment` + `paymentMethod` + `email` persisted; fires confirmation email (non-blocking) for bank transfer; success URL now `/order/success?id=…&method=…`. (5) [OrderSuccessPage.tsx](src/pages/OrderSuccessPage.tsx) — when `method=bank_transfer`, renders a „Bezahlen Sie jetzt per Überweisung“ card with Empfänger/IBAN/BIC/Bank + copyable Verwendungszweck + deadline warning. (6) [AdminOrders.tsx](src/pages/admin/AdminOrders.tsx) — `awaiting_payment` added to status dropdown + orange chip; new green **„Zahlung erhalten“** button for bank-transfer orders awaiting payment → sets status to `processing` and sends `sendPaymentReceivedEmail` (toast confirms sent/not-sent); orange „Vorkasse / Überweisung“ tag in order details. (7) [TrackOrderPage.tsx](src/pages/TrackOrderPage.tsx) — progress steps now depend on `paymentMethod`: bank-transfer orders show „Zahlung ausstehend“ as step 1 instead of „Eingegangen“; awaiting-payment orders display the bank details inline in the expanded card. (8) Status badges/labels updated in [OrdersPage.tsx](src/pages/account/OrdersPage.tsx) + [AdminDashboard.tsx](src/pages/admin/AdminDashboard.tsx); `admin.status.awaitingPayment` translation key added (DE + EN). (9) `.env`/`.env.example` — `VITE_RESEND_API_KEY` + `VITE_EMAIL_FROM` added. eBay keeps its own emails — no integration. `npx tsc --noEmit` — 0 errors; `npm run build` passes. |
